...
import { FileText, Download, FileCheck, FileArchive, Shield, Eye, X, ExternalLink } from 'lucide-react';
...
const mockDocuments = [
  ...
  { id: 26, name: 'Original Certificate of Investment', date: 'Oct 01, 2015', size: '0.5 MB', category: 'certificates', icon: FileCheck, pages: 1 },
  { id: 27, name: 'Certificate of Investment — Foundation Plan', date: 'May 01, 2024', size: '0.8 MB', category: 'certificates', icon: FileCheck, pages: 2 },
  { id: 28, name: 'Certificate of Investment — Additional Deposit', date: 'Mar 01, 2026', size: '0.8 MB', category: 'certificates', icon: FileCheck, pages: 2 },
  { id: 29, name: 'Certificate of Good Standing', date: 'Jun 01, 2026', size: '0.6 MB', category: 'certificates', icon: FileCheck, pages: 1 },
  { id: 30, name: 'Terms and Conditions (v2.1)', date: 'Jan 01, 2026', size: '3.5 MB', category: 'legal', icon: Shield, pages: 18 },
  { id: 31, name: 'Privacy Policy (v2.0)', date: 'Jan 01, 2026', size: '1.2 MB', category: 'legal', icon: Shield, pages: 8 },
  { id: 32, name: 'Investor Agreement — Original', date: 'Oct 01, 2015', size: '2.8 MB', category: 'legal', icon: Shield, pages: 14 },
  { id: 33, name: 'Investor Agreement — Current', date: 'Jan 01, 2024', size: '3.1 MB', category: 'legal', icon: Shield, pages: 16 },
];

function PdfViewer({ doc, onClose }: { doc: typeof mockDocuments[0]; onClose: () => void }) {
      ...
      <div className="bg-card border border-border rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          ...
          <div className="flex items-center gap-2">
            <Badge className="bg-red-500/20 text-red-400 border-red-500/30 text-xs">PDF</Badge>
...
export default function Documents() {
  ...
  const [viewing, setViewing] = useState<typeof mockDocuments[0] | null>(null);
  ...
  const DocumentList = ({ filter }: { filter: string }) => {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {docs.map((doc) => (
          <Card key={doc.id} className="hover:border-primary/40 transition-colors border-border/50">
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div className="flex gap-1 flex-shrink-0">
                <Button variant="ghost" size="icon" className="w-8 h-8 text-muted-foreground hover:text-primary hover:bg-primary/10" onClick={() => setViewing(doc)} title="View PDF">
                  <Eye className="w-4 h-4" />
                </Button>
                <Button variant="ghost" size="icon" className="w-8 h-8 text-muted-foreground hover:text-primary hover:bg-primary/10" onClick={() => handleDownload(doc.name)} title="Download">
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">Access your statements, certificates, tax documents, and legal agreements — dating back to our founding in 2015.</p>
      </div>
